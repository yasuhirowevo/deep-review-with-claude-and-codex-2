...
function buildPrompt(
  ...
  return [
    ...
    "5. finding段ではcoverageを優先し、候補を自己検閲で落とさない。確認済みの事実と成立に必要な未確認の前提を分けて報告する。最終採否・重要度・今回の取扱いはオーケストレーターが別々に裁定します。",
    "6. finding候補ごとに現実的な利用・運用・攻撃・障害からの経路、成立条件の起こりやすさの根拠、既存の予防・検知・復旧後の影響、BASEからの変化と最小修正方針を確認する。未実測でもコードによる立証を使い、Lowの設計・テスト提案は現在の改善価値を示す。",
