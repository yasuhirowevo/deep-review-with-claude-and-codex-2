...
2. 現実的な利用・誤操作・攻撃・障害から到達可能か。成立条件の起こりやすさを何が裏付けるか。
3. 既存のguard、型、transaction、テスト、運用順序、検知・再試行・復旧を考慮しても支障が残るか。
4. 残る影響と重要度が釣り合うか。未実測と成立根拠不足を区別できているか。
5. 修正案が別の互換性・データ・運用問題を作らないか。
6. CI、テスト、PR本文の動作確認、日常の起動経路、Git追跡状態と矛盾しないか。
7. BASEと比べて新規・悪化・既存のどれか。このPRの目的・受け入れ条件に関係するか、妥当な別Issue候補か。
...
成立経路、利用者影響、コード上の根拠を満たす場合だけ採用する。
修正案の裏付け・影響範囲・評価も別途記録し、安全な修正が未確定なら`要設計判断`として扱う。
修正案の未確定を問題の不成立と同一視せず、未検証の修正案を`推奨修正案`にしない。
重要度だけから当PRでの対応要否を決めない。採否は品質契約の「最終指摘への採否」に従う。
成立経路や残る支障を裏付けられない候補は`rejected`、採用済みなら`withdrawn`とし、
`rationale`に確認した事実、不足・反証となる根拠、再評価に必要な証拠を短く記録する。
成立済みの問題は確認済みの証拠に見合う重要度で採用する。未確認点が重要度や対応方針を左右する場合に
Phase 5の取扱いを`additional-verification`とし、確認結果で変わる判断を示す。
判断を変えない未計測値だけでその取扱いにせず、未確認というラベル自体で採否を決めない。
...
オーケストレーターが既存findingも同じ固定入力とPhase 3の採用基準で双方向に再検証して確定する。
